...
def train(showCvScore = True):
    ...
    gridParams = dict(pca__n_components = pca__n_components,
                       clf__gamma = clf__gamma,
                       clf__C = clf__C)
					   
    grid = GridSearchCV(pipeline, gridParams, scoring = scoring,
                        n_jobs = nJobs, refit = True, cv = crossValidator, verbose = 1)
